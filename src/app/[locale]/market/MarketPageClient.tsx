...
export default function MarketPageClient({ market }: MarketPageClientProps) {
  ...
  // ── Map PRODUCT_MAP category IDs → i18n key suffix ──
  ...
  const products = PRODUCT_MAP.map((cat) => {
    const keyPart = CATEGORY_KEY_MAP[cat.id] ?? cat.id;
    const nameKey = `markets.regional.products.items.${keyPart}.name` as Parameters<typeof t>[0];
    const descKey = `markets.regional.products.items.${keyPart}.desc` as Parameters<typeof t>[0];
    const resolvedName = t(nameKey);
    const resolvedDesc = t(descKey);
    return {
      name: resolvedName !== nameKey ? resolvedName : cat.name,
      href: `/products/${cat.id}`,
      desc: resolvedDesc !== descKey ? resolvedDesc : cat.description,
    };
  });
  ...
  const features = [
    {
      ...
      description: t('markets.regional.features.certified_quality_tpl'),
    },
    {
                ...
                <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                  ...
                  <ul className="space-y-3">
                    {[
                      ...
                      { href: '/technical/certifications-compliance-guide', text: t('markets.regional.sidebar.resource_links.gso_saso') },
